...
use PLejeune\UserBundle\Form\PasswordResetForm;
use PLejeune\UserBundle\Form\PasswordResetRequestForm;
...
class PasswordResetController extends Controller
{
    ...
    /**
     * @Route("request.html", name="plejeune_user_password_reset_request")
     * @return Response
     */
    public function request(Request $request, $error = ""): Response
    {
        ...
        $form = $this->createForm(PasswordResetRequestForm::class, NULL, [
            'method' => 'POST',
            'action' => $this->generateUrl("plejeune_user_password_reset_request_confirm"),
        ]);
        $form->handleRequest($request);
        if (!empty($error)) {
            $form->addError(new FormError($error));
        }
        return $this->render('@PLejeuneUser/PasswordReset/request.html.twig', [
            'form' => $form->createView(),
        ]);
    }

    /**
     * @Route("request-confirm.html", name="plejeune_user_password_reset_request_confirm")
     * @return Response
     */
    public function requestConfirm(Request $request): Response
    {
        ...
        $form = $this->createForm(PasswordResetRequestForm::class, NULL, [
            'method' => 'POST',
            'action' => $this->generateUrl("plejeune_user_profile_delete_confirm"),
        ]);
        $form->handleRequest($request);
        ...
        $message = new \Swift_Message($this->get("translator")->trans("mail.reset_password.subject", [], "PLejeuneUser"));
        $message->setFrom("scoopturnfrance@gmail.com", "Scoop Turn");
        $message->setTo($user->getEmail());
        $message->setBody($this->renderView("@PLejeuneUser/Mail/reset_password.html.twig", ["user" => $user]), 'text/html');
        $this->container->get("mailer")->send($message);

        $this->container->get("doctrine")->getManager()->persist($user);
        $this->container->get("doctrine")->getManager()->flush();

        return $this->render('@PLejeuneUser/PasswordReset/request.confirm.html.twig');
    }

    /**
     * @Route("reset-{token}.html", name="plejeune_user_password_reset_form")
     * @return Response
     */
    public function form(Request $request, $token): Response
    {
        ...
        $form = $this->createForm(PasswordResetForm::class, NULL, [
            'method' => 'POST',
            'action' => $this->generateUrl("plejeune_user_password_reset_confirm", ['token' => $token]),
        ]);
        $form->handleRequest($request);

        return $this->render('@PLejeuneUser/PasswordReset/reset.html.twig', [
            'form' => $form->createView(),
        ]);
    }

    /**
     * @Route("confirm-{token}.html", name="plejeune_user_password_reset_confirm")
     * @return Response
     */
    public function formConfirm(Request $request, $token): Response
    {
        ...
        return $this->render('@PLejeuneUser/PasswordReset/reset.confirm.html.twig');
    }
}